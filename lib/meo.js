...
// ===== 出力（GMO 52列様式） =====
// ===== 一括投稿テンプレート（列定義はアプリ側で取り込んだものを Firestore から読む） =====
const TPL_DEFAULT_HEADERS = ["店舗ID", "店舗名", "投稿タイプ", "下書き保存", "即時投稿", "予約投稿日", "予約投稿時間", "Google繰り返し投稿日", "Google繰り返し投稿時間", "Google繰り返し設定_繰り返し", "Google繰り返し設定_間隔", "Google繰り返し設定_曜日（週 選択時）", "Google繰り返し設定_日付（月_日付 選択時）", "Google繰り返し設定_週指定（月_曜日 選択時）", "Google繰り返し投稿終了日（省略可）", "Y_掲載期間の設定（Yahoo!記載欄）の予約投稿日", "Y_掲載期間の設定（Yahoo!記載欄）の予約投稿時間", "Y_掲載期間の設定（Yahoo!記載欄）の予約投稿終了日", "Y_掲載期間の設定（Yahoo!記載欄）の予約投稿終了時間", "Y_概要タブ掲載期間の設定（Yahoo!記載欄）の予約投稿日", "Y_概要タブ掲載期間の設定（Yahoo!記載欄）の予約投稿時間", "Y_概要タブ掲載期間の設定（Yahoo!記載欄）の予約投稿終了日", "Y_概要タブ掲載期間の設定（Yahoo!記載欄）の予約投稿終了時間", "A_掲載期間の設定（Apple記載欄）の予約投稿日 ", "A_掲載期間の設定（Apple記載欄）の予約投稿終了日", "タイトル(GoogleとSNSは任意入力、Yahoo!は必須）", "投稿内容", "投稿内容２", "画像", "クーポンコード（省略可）", "特典利用へのリンク", "利用規約", "ボタンの追加", "ボタンURL", "A_アクションボタン（Apple）", "ハッシュタグ", "開始日", "開始時間", "終了日", "終了時間", "【Google】繰り返し頻度", "【Google】繰り返し指定曜日", "【Google】繰り返し投稿の終了日", "Facebook", "Instagram", "X（旧Twitter）", "Yahoo!", "Apple", "エキテン", "エ_投稿カテゴリー", "エ_掲載開始日（エキテン記載欄）", "エ_掲載開始時間（エキテン記載欄）", "エ_掲載終了日（エキテン記載欄）", "エ_掲載終了時間（エキテン記載欄）", "エ_タイムライン通知"];
function normHdr(v) {
  return String(v == null ? '' : v).normalize('NFKC').replace(/\s+/g, '').replace(/（/g, '(').replace(/）/g, ')').trim();
}
const FIELD_SPECS = [
  { key: 'storeId', label: '店舗ID', req: true, match: h => h === '店舗ID' },
  { key: 'storeName', label: '店舗名', req: true, match: h => h === '店舗名' },
  { key: 'postType', label: '投稿タイプ', req: true, match: h => h === '投稿タイプ' },
  { key: 'immediate', label: '即時投稿', req: true, match: h => h === '即時投稿' },
  { key: 'schedDate', label: '予約投稿日', req: false, match: h => h === '予約投稿日' },
  { key: 'schedTime', label: '予約投稿時間', req: false, match: h => h === '予約投稿時間' },
  { key: 'title', label: 'タイトル', req: true, match: h => h.indexOf('タイトル') === 0 },
  { key: 'body', label: '投稿内容', req: true, match: h => h === '投稿内容' },
  { key: 'image', label: '画像', req: true, match: h => h === '画像' },
  { key: 'btnType', label: 'ボタンの追加', req: true, match: h => h === 'ボタンの追加' },
  { key: 'btnUrl', label: 'ボタンURL', req: true, match: h => h === 'ボタンURL' },
  { key: 'evStart', label: '開始日', req: false, match: h => h === '開始日' },
  { key: 'evStartTime', label: '開始時間', req: false, match: h => h === '開始時間' },
  { key: 'evEnd', label: '終了日', req: false, match: h => h === '終了日' },
  { key: 'evEndTime', label: '終了時間', req: false, match: h => h === '終了時間' },
];
function resolveCols(headers) {
  const norm = (headers || []).map(normHdr);
  const map = {}, missing = [], missingOpt = [];
  FIELD_SPECS.forEach(f => {
    const i = norm.findIndex(h => h && f.match(h));
    if (i >= 0) map[f.key] = i; else (f.req ? missing : missingOpt).push(f.label);
  });
  return { map, missing, missingOpt };
}
// アプリ（設定タブ）で取り込んだテンプレートを共有で読む。無ければ既定55列
async function loadBulkTemplate() {
  let rec = null;
  try { rec = await getDoc('studio_settings', 'bulk_template'); } catch (e) {}
  const headers = (rec && Array.isArray(rec.headers) && rec.headers.length) ? rec.headers : TPL_DEFAULT_HEADERS.slice();
  return { headers, newline: (rec && rec.newline) || 'real', version: (rec && rec.version) || '既定（55列）', cols: resolveCols(headers) };
}
...
// 2026/10/01以降の配信カデンス：毎月1日（8:30）＋火8:30／金17:00
function cadenceTime(dateStr) {
  if (!dateStr || dateStr < '2026-10-01') return '';
  if (String(dateStr).slice(8) === '01') return '08:30';
  const dow = new Date(dateStr + 'T00:00:00Z').getUTCDay();
  if (dow === 2) return '08:30';
  if (dow === 5) return '17:00';
  return '';
}
...
function buildRowsFrom(d, STORES, tpl) {
  const T = tpl || { headers: TPL_DEFAULT_HEADERS, newline: 'real', cols: resolveCols(TPL_DEFAULT_HEADERS) };
  const M = T.cols.map;
  const bodyOut = (t) => { const v = String(t == null ? '' : t); return T.newline === 'literal' ? v.replace(/\r?\n/g, '\\n') : v; };
  const rows = [];
  exportStoresFrom(d, STORES).forEach(s => {
    ...
    const c = new Array(T.headers.length).fill('');
    const put = (k, v) => { if (M[k] != null) c[M[k]] = v; };
    put('storeId', s.gmo); put('storeName', s.name); put('postType', d.postType || '最新情報');
    if (immediate) put('immediate', '○'); else { put('schedDate', date ? fmtDate(date) : ''); put('schedTime', time ? fmtTime(time) : ''); }
    put('title', d.title || '');
    put('body', bodyOut(ov.content !== undefined ? ov.content : contentFromFor(d, s)));
    put('image', safeImgName(ov.imageTitle !== undefined ? ov.imageTitle : (d.imageTitle || '')));
    const b = btnFor(d, s); put('btnType', b.type); put('btnUrl', b.url);
    if (d.postType === 'イベント' || d.postType === 'クーポン') {
      ...
      put('evStart', fmtDate(evs)); put('evStartTime', fmtTime(d.evStartTime || hr.st));
      put('evEnd', fmtDate(eve)); put('evEndTime', fmtTime(d.evEndTime || hr.et));
      // SNS列は空欄のまま（✕を入れると画像エラーになった実績あり）
    }
    rows.push(c);
  });
  return rows;
...
function csvFrom(rows, tpl) {
  const T = tpl || { headers: TPL_DEFAULT_HEADERS };
  const q = v => `"${String(v == null ? '' : v).replace(/"/g, '""')}"`;
  return '\ufeff' + [T.headers.map(q).join(',')].concat(rows.map(r => r.map(q).join(','))).join('\r\n') + '\r\n';
}
function xlsxFrom(rows, tpl) {
  const T = tpl || { headers: TPL_DEFAULT_HEADERS, cols: resolveCols(TPL_DEFAULT_HEADERS) };
  const M = T.cols.map;
  const XLSX = require('xlsx');
  const aoa = [T.headers.slice()].concat(rows.map(r => r.map((v, i) => {
    if (v == null || v === '') return null;
    if (i === M.storeId && /^\d+$/.test(String(v))) return Number(v);
    return v;
  })));
  const ws = XLSX.utils.aoa_to_sheet(aoa);
  ws['!cols'] = T.headers.map((_, i) => i === M.body ? { wch: 60 } : ((i === M.storeName || i === M.title) ? { wch: 26 } : { wch: 14 }));
...
module.exports = {
  ...
  TPL_DEFAULT_HEADERS, resolveCols, loadBulkTemplate, BTN_TYPES, buildRowsFrom, exportStoresFrom, contentFromFor, csvFrom, xlsxFrom, mmddFrom,
  safeImgName, validate, NG, uid, todayISO, nowStr, schedSummary, SCHED_FIELDS, cadenceTime,
};
